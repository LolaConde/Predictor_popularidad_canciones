...
De este modo, se generan un conjunto de árboles de regresión que se usarán en la predicción tomando la predicción de cada uno de ellos y combinándola para obtener la predicción final.
...
La parte aleatoria de la creación del modelo permite que el modelo no se sobreajuste a los datos de entrenamiento como ocurría al generar un árbol de decisión o regresión, y por lo tanto, permite una mejor generalización.
...
Como se dijo anteriormente, en el entrenamiento de cada árbol, en cada nodo, se selecciona de forma aleatoria un subconjunto de predictores, y de ellos se elije el mejor para dividir el nodo. Este subconjunto es del tamaño que indica este parámetro *mtry*, y la decisión de qué predictor elegir se hace siguiendo el método indicado en *splitrule*. Por último, *min.node.size* indica el número mínimo de muestras que debe tener un nodo para poder dividirse.
...
Una vez creado el conjunto de datos de prueba, se entrena el modelo con el grid de hiperparámetros creado. Para ello, se deben de incluir el número de árboles del bosque, por lo que se va a probar con 50, 100, 200, 300, y 400 árboles.
...
for (mtry_i in 3:7) {
  ...
  plots[[mtry_i - 2]] <- ggplot(data=datos_mtry_3) +
...
```
Se observa que el Rsquared es 0.85, lo que indica que lo que el 85% de la variabilidad de *popularity* se puede explicar con el modelo. El RMSE es de 5.9, y el MAE es de 3.6, valores bajos teniendo en cuenta que *popularity* va de 0 a 100. Por lo tanto, se puede concluir que se obtiene un buen modelo.
...
Aunque parezca que existen muchas variables sin importancia, es importante resaltar que la mayoría de estas es la misma (key). Por lo tanto, el modelo creado considera importantes muchos de los predictores, aunque no todos.