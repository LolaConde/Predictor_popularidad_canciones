...
rutaDeDatos <- ""
rutaRDS <- "rds/"
datos <- read.csv(paste(rutaDeDatos, "spoti.csv", sep=""), na.strings = "")
```

Una vez almacenado el dataset que se encontraba en la carpeta *rutaDeDatos* en *datos*, se procede describiendo este dataset.

A su vez, se ha declarado la ruta a la carpeta donde se encuentran los ficheros rds, que evitan ejecutar partes largas del código como entrenar modelos.
...
ranger_50_trees <- train(popularity ~ ., # Popularity es la variable a predecir, el resto son predictores
   ...
   num.trees=50,
   importance='impurity')
saveRDS(ranger_50_trees, paste(rutaRDS, "ranger_50_trees.rds", sep=""))
```

Se guardan los resultados en archivos RDS para no tener que entrenar de nuevo los modelos una vez ya entrenados.

```{r}	
ranger_50_trees <- readRDS(paste(rutaRDS, "ranger_50_trees.rds", sep=""))
```
...
ranger_100_trees <- train(popularity ~ ., # Popularity es la variable a predecir, el resto son predictores
  ...
  num.trees=100,
  importance='impurity')
saveRDS(ranger_100_trees, paste(rutaRDS, "ranger_100_trees.rds", sep=""))
```

```{r}
ranger_100_trees <- readRDS(paste(rutaRDS, "ranger_100_trees.rds", sep=""))
```
...
ranger_200_trees <- train(popularity ~ ., # Popularity es la variable a predecir, el resto son predictores
  ...
  num.trees=200,
  importance='impurity')
saveRDS(ranger_200_trees, paste(rutaRDS, "ranger_200_trees.rds", sep=""))
```

```{r}
ranger_200_trees <- readRDS(paste(rutaRDS, "ranger_200_trees.rds", sep=""))
```
...
ranger_300_trees <- train(popularity ~ ., # Popularity es la variable a predecir, el resto son predictores
  ...
  num.trees=300,
  importance='impurity')
saveRDS(ranger_300_trees, paste(rutaRDS, "ranger_300_trees.rds", sep=""))
```

```{r}
ranger_300_trees <- readRDS(paste(rutaRDS, "ranger_300_trees.rds", sep=""))
```
...
ranger_400_trees <- train(popularity ~ ., # Popularity es la variable a predecir, el resto son predictores
  ...
  num.trees=400,
  importance='impurity')
saveRDS(ranger_400_trees, paste(rutaRDS, "ranger_400_trees.rds", sep=""))
```

```{r}
ranger_400_trees <- readRDS(paste(rutaRDS, "ranger_400_trees.rds", sep=""))
``` 
...
saveRDS(final_random_forest, paste(rutaRDS, "final_random_forest.rds", sep=""))
```

```{r}
final_random_forest = readRDS(paste(rutaRDS, "final_random_forest.rds", sep=""))
```
...
- *size*: Se suele probar con números cercanos al número de predictores. Dado que se tienen 34 (por transformar las variables categóricas a numéricas), se va a probar con 25, 35 y 45 neuronas en la capa oculta.
...
mygridmlp = expand.grid(size =  c(25, 35, 45),
...
Una vez creado el grid, se entrenan los modelos con los distintos hiperparámetros, de forma que se encuentren los que mejor funcionan para nuestros datos. Se va a probar con un conjunto de datos más pequeño (25%):

```{r}
set.seed(123)
elementos_datos_prueba_dl = sample(1:nrow(datos_dl), nrow(datos_dl)*0.25)
...
saveRDS(modelos_dl, paste(rutaRDS, "modelos_dl.rds", sep=""))
```

```{r}
modelos_dl <- readRDS(paste(rutaRDS, "modelos_dl.rds", sep=""))
```
...
saveRDS(pred.test, paste(rutaRDS, "predTest_prueba_dl.rds", sep=""))
```

Y se muestran los resultados:

```{r}
pred.test <- readRDS(paste(rutaRDS, "predTest_prueba_dl.rds", sep=""))
postResample(as.numeric(datos_prueba_test_dl$popularity), as.numeric(pred.test))
```
Como se puede observar, el modelo tiene un Rsquared de 0.70 y un MAE y RMSE de 5.2 y 8 respectivamente. Se observan relativamente buenos resultados, pero se espera a obtener el modelo con todos los datos para comprobarlo.
...
saveRDS(final_modelo_dl, paste(rutaRDS, "final_modelo_dl.rds", sep=""))
```

```{r}
final_modelo_dl = readRDS(paste(rutaRDS, "final_modelo_dl.rds", sep=""))
```
...
saveRDS(pred.test, paste(rutaRDS, "predTest_dl.rds", sep=""))
```

```{r}
pred.test <- readRDS(paste(rutaRDS, "predTest_dl.rds", sep=""))
postResample(as.numeric(datos_test_dl$popularity), as.numeric(pred.test))
```
Se observa una mejora en el *Rsquared* en relación con el modelo entrenado con pocos datos, y tanto el MAE como el RMSE han disminuido. Esto es coherente, ya que disponer de más datos para entrenar generalmente conduce a mejores resultados.

El modelo de red neuronal puede explicar (*rsquared*) un 77.8% de la variabilidad de la variable popularity, obteniendo un MAE y un RMSE de 4.3 y 6.8 respectivamente. Estas métricas se consideran relativamente buenas.

Sin embargo, las métricas del modelo de red neuronal son significativamente peores en comparación con las obtenidas mediante árboles de decisión. Esto se verá en más profundidad en el apartado D.
...
saveRDS(svm_modelos, paste(rutaRDS, "svm_modelos.rds", sep=""))
```

Se guardan los resultados en archivos RDS para no tener que entrenar de nuevo los modelos una vez ya entrenados.

```{r}	
svm_modelos <- readRDS(paste(rutaRDS, "svm_modelos.rds", sep=""))
```
...
saveRDS(final_svm, paste(rutaRDS, "final_svm.rds", sep=""))
```

```{r}
final_svm = readRDS(paste(rutaRDS, "final_svm.rds", sep=""))
```
...
Como se ha podido observar, el modelo tiene un Rsquared de 0.97, por lo que el modelo puede explicar un 97% de la variablidad de *popularity*. A su vez, los errores son de 1.2 y 2.5 en MAE y RMSE respectivamente. Por lo tanto, el modelo da buenos resultados para ejemplos no vistos.
...
Como se puede observar, la red neuronal (DeepLearning), obtiene un error mucho mayor en relación a los otros dos modelos. A su vez, Rsquared es menor. Aunque la red neuronal da buenos resultados, no se considera buen modelo en relación con los otros.
